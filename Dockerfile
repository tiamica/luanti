...
# 1. Force root user so we have permission to install packages
USER root

# 2. Install dependencies and create the minetest user
...
# 3. Download the game using the newly installed curl
...
# 4. Copy the compiled server files from the builder stage
...
# 5. NOW we switch to the restricted user for security, right before starting the server
